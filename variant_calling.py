...
def variant_calling_varscan2(task_name, base_path, aligner_list):
    ...
    mpileup2cns_cmd = ['java', '-jar', '/home/leftc/bioapp/varscan2/VarScan.v2.4.4.jar', 'mpileup2cns']
    ...
    for aligner in aligner_list:
        ...
        ref_path = aln_data_cwd.joinpath(task_name+'_ref.fasta')
        ref_cmd = ['-f', str(ref_path)]
        output_path = str(aln_data_cwd.joinpath('%s_%s_varscan.vcf'%(task_name, aligner)))
...
def build_vc_summary_json(task_name, base_path, aligner_list):
    vc_summary_path = base_path.joinpath(task_name, task_name+'_vc_summary.json')
    varscan_vc_dict = {}
    lofreq_vc_dict = {}
    for aligner in aligner_list:
        # parse varscan vcf file
        varscan_vcf_file_path = base_path.joinpath(task_name, 'alignment', aligner, '%s_%s_varscan.vcf'%(task_name, aligner))
        ...
        for pos_result in original_varscan_vc_dict['vc']:
            if pos_result[4] != '.':
                ...
                varscan_vc_dict[pos]['SNV'][alt][aligner] = {'FILTER': vc_filter, 'FREQ': freq, 'QUAL': qual, 'DP': sdp}
        # parse lofreq vcf file
        lofreq_vcf_file_path = base_path.joinpath(task_name, 'alignment', aligner, '%s_%s_lofreq.vcf'%(task_name, aligner))
        ...
        for pos_result in original_lofreq_vc_dict['vc']:
            ...
            if Decimal(af) >= Decimal('0.1'):
                ...
                lofreq_vc_dict[pos]['SNV'][alt][aligner] = {'FILTER': vc_filter, 'FREQ': af_formated, 'QUAL': qual, 'DP': dp}
