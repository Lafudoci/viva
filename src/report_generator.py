    ...
    if task.remove_impurities != None:
        for impurit_order in range(1, task.impurities_prefilter_num+1):
            impurit_m_c += '\n'.join([
                ...
                '| Aligner | Overall mapped rate | Primary mapped reads | Covered base |',
                '| ------- | ------------------- | -------------------- | ------------ |',
                '| Bowtie2 | %s | %s | %s %% |'%(s['impurit_filter_results'][str(impurit_order)]['bt2']['remove_percentage'],s['impurit_filter_results'][str(impurit_order)]['bt2']['mapped_reads'], s['impurit_filter_coverage'][str(impurit_order)]['bt2']['coverage']),
                '| BWA MEM | %s | %s | %s %% |'%(s['impurit_filter_results'][str(impurit_order)]['bwa']['remove_percentage'],s['impurit_filter_results'][str(impurit_order)]['bwa']['mapped_reads'], s['impurit_filter_coverage'][str(impurit_order)]['bwa']['coverage']),
            ])
    else:
